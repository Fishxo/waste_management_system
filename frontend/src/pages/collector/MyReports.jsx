      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 max-w-2xl">
            Complaints and waste-related issues you reported. Only{' '}
            <span className="font-medium text-gray-700">pending</span>{' '}
            reports can be edited or deleted.
          </p>
        </div>
        <Link
          to="/collector/reports/create"
          className="inline-flex items-center justify-center bg-sky-600 hover:bg-sky-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition shrink-0"
        >
          + Create Report
        </Link>
      </div>
      ...
      {filteredReports.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600">
            {filter ? `No ${formatStatus(filter)} reports found.` : 'No reports found.'}
          </p>
          <p className="text-sm text-gray-400 mt-1">
            ...
            for waste problems or complaints you encountered.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100">
                  <Link
                    to={`/collector/reports/${reportId}`}
                    className="inline-flex items-center justify-center bg-sky-600 hover:bg-sky-700 text-white py-2 px-4 rounded-lg text-sm font-medium transition"
                  >
                    View Details
                  </Link>
                  {isPending && (
                    <>
                      <Link
                        to={`/collector/reports/${reportId}/edit`}
                        className="inline-flex items-center justify-center bg-white border border-sky-200 text-sky-700 hover:bg-sky-50 py-2 px-4 rounded-lg text-sm font-medium transition"
                      >
                        Edit
                      </Link>
                      <button
